...
An answer that stops mid-sentence reached the output token limit; raise **max output tokens**. ComfyUI's cancel stops waiting, but tokens the model produced before a cancel are billed. Image file types without transparency give no masks here; for masks, use **Image: Generate**. ComfyUI reuses a cached answer when nothing changed; the seed changes after each run unless its control is **fixed**.
