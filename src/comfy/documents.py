...
from ..state.chat import Document
...
def read_document_file(name: str, max_megabytes: int) -> Document:
    ...
    if not path.is_relative_to(folder) or not path.is_file():
        raise ConnectorError(ErrorCode.INVALID_INPUT, DOCUMENT_OUTSIDE)
    media_type = DOCUMENT_TYPES.get(path.suffix.lower())
    if media_type is None:
        raise ConnectorError(ErrorCode.INVALID_INPUT, DOCUMENT_KIND)
    if path.stat().st_size > max_megabytes * BYTES_PER_MEBIBYTE:
        raise ConnectorError(ErrorCode.INVALID_INPUT, DOCUMENT_SIZE.format(maximum=max_megabytes))
    ...
    try:
        ...
        raise ConnectorError(ErrorCode.INVALID_INPUT, DOCUMENT_UNREADABLE) from None
