...
from ..types.models import Model, ModelReply
...
async def validate_model(model_id: str, endpoint: Endpoint, configuration: Configuration) -> Model:
    ...
    if model is None:
        content = await download_listing(model_id, configuration) if MODEL_ID.match(model_id) else None
        if content is None:
        ...
        model = _MODELS[model_id] = Model(frozenset(listing.architecture.output_modalities), frozenset(parameters))
