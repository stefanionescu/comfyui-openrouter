...
from ..state.models import CheckedModel, ModelReply
...
async def check_model(model_id: str, endpoint: Endpoint, configuration: ExecutionConfiguration) -> CheckedModel:
    ...
    if not model_id:
        raise ConnectorError(ErrorCode.INVALID_INPUT, MODEL_EMPTY)
    model = _MODELS.get(model_id)
    if model is None:
        content = await get_model(model_id, configuration) if MODEL_ID.match(model_id) else None
        if content is None:
            raise ConnectorError(ErrorCode.INVALID_INPUT, MODEL_UNKNOWN.format(model=model_id))
        try:
    ...
    if model.outputs.isdisjoint(MODEL_OUTPUTS[endpoint]):
        ...
        raise ConnectorError(ErrorCode.INVALID_INPUT, MODEL_KIND.format(model=model_id, kind=kind))
    return model
