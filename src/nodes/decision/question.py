...
from ...state.decisions import QuestionSet, ScoreQuestion, YesNoQuestion, ChoiceQuestion
...
if TYPE_CHECKING:
    ...
    from ...state.decisions import Question
...
def _build_choice(name: str, instructions: str, text: str) -> ChoiceQuestion:
    ...
    for line in (line.strip() for line in text.splitlines()):
        ...
        if any(key.strip() == known for known, _description in options):
            raise ConnectorError(ErrorCode.INVALID_INPUT, CHOICE_OPTION_REPEATED.format(key=key.strip()))
...
def _build_question(name: str, instructions: str, answer_type: Mapping[str, object]) -> Question:
    ...
    if bool(yes_means) != bool(no_means):
        raise ConnectorError(ErrorCode.INVALID_INPUT, YES_NO_CRITERIA)
    ...
    def execute(  # pyright: ignore[reportIncompatibleMethodOverride] -- reason: ComfyUI calls by schema.
        ...
        *,
        ...
        if QUESTION_NAME_TEXT.match(name) is None:
            raise ConnectorError(ErrorCode.INVALID_INPUT, QUESTION_NAME)
        if any(question.name == name for question in earlier):
            raise ConnectorError(ErrorCode.INVALID_INPUT, QUESTION_REPEATED.format(name=name))
        if not instructions.strip():
            raise ConnectorError(ErrorCode.INVALID_INPUT, QUESTION_INSTRUCTIONS)
        if len(earlier) >= MAX_QUESTIONS:
            raise ConnectorError(ErrorCode.INVALID_INPUT, QUESTION_LIMIT.format(maximum=MAX_QUESTIONS))
